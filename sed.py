...
hPlanck    = 6.6260755e-27 # erg*s (added by ASL)
...
class SED:
    ...
    freq_range = energy_range_erg/hPlanck # frequency in Hz, added by ASL
    ...
    def eddington_luminosity(self):
        ...
        M : float
            Black Hole Mass in solar Masses
        """
        # Ledd = (4*np.pi*c*G*m_p*Mbh)/sigma_t
    ...
    def disk_spectral_luminosity_truncated(self, energy,rmin): # ADDED BY ASL -------------------------------------------------------------------------------------------
        """
        disk spectral luminosity in units of 1 / s.
        ...
        rmin in cm
        """
        radial_integral = 2 * np.pi**2 * self.Rg**2 * integrate.quad( lambda r: r * self.disk_spectral_radiance(energy,r), rmin, self.gravity_radius)[0]
        spectral_lumin = 2 * radial_integral # 2 sides of the disk
        return spectral_lumin
    ...
    def disk_sed_truncated(self,rmin): # ADDED BY ASL -----------------------------------------------------------------------------------------------------------------
        """
        disk SED in energy units.
        EL_E[ KeV KeV / s / KeV]
        ...
        lumin = []
        for energy_erg in energy_range_erg:
            lumin.append(self.disk_spectral_luminosity_truncated(energy_erg,rmin))
        sed = np.array(lumin) * self.energy_range
        return sed
    
    def disk_Lnu_truncated(self,rmin): # ADDED BY ASL -----------------------------------------------------------------------------------------------------------------
        """
        truncated luminosity L_nu of disk, [erg/s/Hz]
        """
        disk_Lnu_trunc = self.disk_sed_truncated(rmin)*hPlanck*self.freq_range/(self.energy_range)**2/(2.418e17)
        return disk_Lnu_trunc
    ...
    def disk_flux_truncated(self, rmin, distance): # ADDED BY ASL ------------------------------------------------------------------------------------------------------
        """
        Flux of the disk component in units of keV^2 ( Photons / cm^2 / s / keV).
        ...
        rmin in cm
        """
        return self.disk_sed_truncated(rmin) / (4*np.pi*distance**2)
    ...
    def corona_radius(self):
        ...
        try:
            ...
            print("Accretion rate is too low --> corona radius > self gravity radius")
    ...
    def total_flux(self, distance):
        ...
        corona_flux = self.corona_flux(distance)  # originally corona_flux=self.warm_flux(distance) --> I fixed typo (ASL)
        total_flux = disk_flux + warm_flux + corona_flux
        return total_flux
    
    def disk_flux_erg(self, distance): # convert flux units to erg/cm^2/s/Hz (added by ASL)
        disk_erg = (self.disk_flux(distance)/(self.energy_range**2)/2.417990504024e+17)*(hPlanck*self.freq_range)
        return disk_erg

    def disk_flux_erg_truncated(self, rmin, distance): # convert flux units to erg/cm^2/s/Hz (added by ASL)
        disk_erg_trunc = (self.disk_flux_truncated(rmin,distance)/(self.energy_range**2)/2.417990504024e+17)*(hPlanck*self.freq_range)
        return disk_erg_trunc
    
    def warm_flux_erg(self, distance): # convert flux units to erg/cm^2/s/Hz (added by ASL)
        warm_array = np.asarray(self.warm_flux(distance))
        warm_erg = (warm_array/(self.energy_range**2)/2.417990504024e+17)*(hPlanck*self.freq_range)
        return warm_erg

    def corona_flux_erg(self, distance): # convert flux units to erg/cm^2/s/Hz (added by ASL)
        corona_erg = (self.corona_flux(distance)/(self.energy_range**2)/2.417990504024e+17)*(hPlanck*self.freq_range)
        return corona_erg



    def total_flux_erg(self, distance): # convert flux units to erg/cm^2/s/Hz (added by ASL)
        """
        Total flux at distance, in erg/cm^2/s/Hz.
        ...
        total_flux_erg = (total_flux/(self.energy_range**2)/2.417990504024e+17)*(hPlanck*self.freq_range)
        
        return total_flux_erg
    ...
    def plot_total_only(self, distance, ax = None): # Added by ASL
        """
        Plot total flux in energy units.
        yaxis: EL_E [ keV keV / s / keV]
        xaxis: E [keV]
...
## Plots of converted unit fluxes (All added by ASL) -----------------------------------------------------------------------------------------------------------

    def plot_total_erg(self, distance, ax = None): # Added by ASL
        """
        Plot total flux.
        xaxis: frequency [Hz]
        yaxis:  erg / cm^2 / s / Hz
        ...
        if(ax is None):
            print("creating figure.")
            fig, ax = plt.subplots()
        flux = self.total_flux_erg(distance)
        ax.loglog(self.freq_range, flux, color = next(colors), linewidth=4, label = 'Total')
        # ax.loglog(self.freq_range, flux, linewidth=2,color=next(colors))#, label = 'Total')


        # ax.set_ylim(np.max(flux)* 1e-2, 2*np.max(flux))
        ax.set_xlabel(r"Frequency $\nu$ [ Hz ]")
        ax.set_ylabel(r"erg / cm$^{2}$ / s / Hz")
        # ax.legend()
        return ax

    def plot_disk_erg(self, distance, color = 'b', ax = None, label = None):
        """
        Plot disk flux in energy units.
        xaxis: frequency [Hz]
        yaxis:  erg / cm^2 / s / Hz
        ...
        flux = self.disk_flux_erg(distance)
        
        if(ax is None):
            fig, ax = plt.subplots()
            
        ax.loglog(self.freq_range, flux, color = color, label=label, linewidth = 2)
        ax.set_ylim(np.max(flux) * 1e-2, 2*np.max(flux))
        ax.set_xlabel(r"Frequency $\nu$ [ Hz ]")
        ax.set_ylabel(r"erg / cm$^{2}$ / s / Hz")
        return ax

    def plot_corona_erg(self, distance, color = 'b', ax = None, label = None):
        """
        Plot corona flux in energy units.
        xaxis: frequency [Hz]
        yaxis:  erg / cm^2 / s / Hz
        ...
        flux = self.corona_flux_erg(distance)
        if(ax is None):
            fig, ax = plt.subplots()
        ax.loglog(self.freq_range, flux, color = color, label=label, linewidth = 2)
        ax.set_ylim(max(flux) * 1e-2, 2*max(flux))
    ...
    def plot_warm_erg(self, distance, color = 'b', ax = None, label = None):
        """
        Plot warm component flux in energy units.
        xaxis: frequency [Hz]
        yaxis:  erg / cm^2 / s / Hz
        ...
        flux = self.warm_flux_erg(distance)
        if(ax is None):
            fig, ax = plt.subplots()
        ax.loglog(self.freq_range, flux, color = color, label=label, linewidth = 2)
        ax.set_ylim(np.max(flux) * 1e-2, 2*np.max(flux))
    ...
    def plot_all_erg(self, distance, ax = None):
        """
        Plot total flux in energy units.
        xaxis: frequency [Hz]
        yaxis:  erg / cm^2 / s / Hz
        ...
        if(ax is None):
            print("creating figure.")
            fig, ax = plt.subplots()
        flux = self.corona_flux_erg(distance) + self.disk_flux_erg(distance) + self.warm_flux_erg(distance)
        ax.loglog(self.freq_range, flux, color = next(colors), linewidth=4, label = 'Total')
        ax = self.plot_disk_erg(distance, color = next(colors), ax=ax, label = 'Disk component')
        ax = self.plot_corona_erg(distance, color = next(colors), ax=ax, label = 'Corona component')
        ax = self.plot_warm_erg(distance, color = next(colors), ax=ax, label = 'Warm component')
        
        # ax.set_ylim(np.max(flux)* 1e-2, 2*np.max(flux)) # original y-axis limits
        ax.set_ylim(self.warm_flux_erg(distance)[1],2*np.max(flux)) # new y-axis limits
        ax.set_xlabel(r"Frequency $\nu$ [ Hz ]")
        ax.set_ylabel(r"$F_{\nu}$ [erg / cm$^{2}$ / s / Hz]")
        ax.legend()
        return ax
